...
| Low | NotebookLM auth dead 6 cron runs | `No cookies found` on 2026-06-15, -16, -17, -19, -20, -21. Needs `npx oneclicklm login` re-run by Roy (interactive). Until then, web tools are the primary cron research path (skill prerequisite updated). Documented in [[40-Resources/model-context-protocol\|MCP page]] as a credential, not protocol, failure. |
