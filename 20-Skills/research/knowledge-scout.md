...
domain: "research"
prerequisites:
  - "WebSearch + WebFetch available (PRIMARY research path for cron — NotebookLM auth has failed 5 consecutive cron runs)"
  - "NotebookLM MCP tools available (preferred only for interactive runs where a human can re-auth — auth has failed 6 consecutive cron runs)"
  - "Obsidian vault open"
